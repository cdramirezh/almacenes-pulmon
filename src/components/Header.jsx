...
const Header = ({ supplierData, setSupplierData, menuData }) => {
	...
	useEffect(() => {
		...
		$mobileMenuLinks.forEach($link =>
			$link.addEventListener("click", () => setShowMobileMenu(false))
		);
	}, [supplierData]);
		...
		<header className="header">
			<Container fluid>
				<Row>
					<Col className="left">
						<Link to="/profile">
							<img src="https://decorceramica.wpengine.com//LogoDECORBLANCO2024.png" alt="Decor" />
						</Link>
					</Col>
											...
											{menuData.map(menuItem => (
												<React.Fragment key={menuItem.title}>
													{menuItem.target === "/login" ? (
														<NavDropdown.Divider key="separator" />
													) : null}
													<LinkContainer
														to={menuItem.target}
														onClick={
													...
													>
														<NavDropdown.Item>{menuItem.title}</NavDropdown.Item>
													</LinkContainer>
												</React.Fragment>
											))}
										</NavDropdown>
