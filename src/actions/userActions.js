...
export const forgotPasswordSendEmail = async (idType, idNumber) => {
    ...
    if(error) {
        if(error.response.status === 400) {
            throw new Error('Usuario no encontrado, valide tipo y número de identificación')    
        } if(error.response.status === 500 && error.response.data.error === "No se encontró email") {
            throw new Error('Usuario no tiene correo electrónico registrado. Enviar correo a contabilidad@grupodecor.com')    
        }
...
export const getUserFromHexCode = async ({ validationCode }) => {
	const res = await axios.get(`${process.env.REACT_APP_URL_API_MOVILIDAD}/delivery_re/${validationCode}`).catch(() => {});
	return res?.data?.USER_NAME;
};

export const registerBeforeConfirmationEmail = async (
	idType,
	idNumber,
	account
) => {
	const username = `ESP${idType}${idNumber}`;
	const hex = Math.floor(Math.random() * 100000000000000000000)
		.toString(16)
		.toUpperCase();

	// Seng email
	const { data: {info: {recipient}}, error } = await requestToSAP(
		`${process.env.REACT_APP_URL_API_ERP}/clovit/ws_rest?sap-client=300&method=GET_SUPPLIER_DATA&object=Account`,
		{url_payload: {
			ID: idNumber,
			...
			account: account,
			hex_cod: hex,
			preferedTemplate: 'proveedores'
		},
		'validateUser'
		)

	// const { data: {info: {recipient}}, error } = await axios
	// 	.post('/validateUser', {
	// 		account: account,
	// 		hex_cod: hex,
	// 	})
	...
	if (error) {
		if (error.response.status === 400) {
			throw new Error(
				"Usuario no encontrado, valide tipo y número de identificación"
			);
		}
		if (
			error.response.status === 500 &&
			error.response.data.error === "No se encontró email"
		) {
			throw new Error(
				"Usuario no tiene correo electrónico registrado. Enviar correo a contabilidad@grupodecor.com"
			);
		}
		throw new Error("Ha ocurrido un error al enviar el correo electrónico");
	}
	...
	// Set record
	const { error2 } = await axios
		.post(`${process.env.REACT_APP_URL_API_MOVILIDAD}/delivery_re`, {
			USER_NAME: username,
			HEX_COD: hex,
			STATUS: "D",
			TODAY: new Date().toISOString().substring(0, 10),
